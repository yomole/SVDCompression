...
void Scene::deserialize(const string &fileLocation){
    //1. Give the name of the scene (isolate the filename).
    ...
    if (file.is_open()){
        ...
        while(getline(file, line)){
            ...
            //Comment in the scene starts with '#'
...
void Scene::addCheckbox(istringstream& parse){
    string elementName, textureName, xpos, ypos, checkedS, functionName;
    ...
    //Convert position to Vector2fs.
    Vector2f position(stof(xpos), stof(ypos));

    //Convert checked into bool.
    bool checked;
    ...
    if (checkedS == "true"){
        ...
        checked = false;
        cerr << "Checkbox " << elementName << "'s checked value was initialized incorrectly! Using checked = false..." << endl;
    }

    //Create function for checkbox.
    function<void(vector<string>&)> function = getFunction(nameAndArgs);

    //Create checkbox and add to scene set.
    auto* newCheckbox = new Checkbox(elementName, AssetManager::getTexture(textureName), position, function, nameAndArgs, checked);
    this->checkboxes.insert(pair<string, Checkbox*>(elementName, newCheckbox));
}
